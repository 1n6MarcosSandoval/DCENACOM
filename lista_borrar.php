...
$rowGEOR = oci_fetch_array($stid1, OCI_ASSOC+OCI_RETURN_NULLS);
//print "Evento: ".$row1["ID_EVENTO"]."<br/><br/>";
/* FIN Selecciona el evento del reporte*/

/*************Tabla GEOR*********************/
/*Busqueda ID_REPORTE DB.GEOR****************/
...
/**/
/*Borrar Estados y municipios*/

		if($rowTipoReporte["ID_TIPO_REPORTE"]==0 || $rowTipoReporte["ID_TIPO_REPORTE"]==2){

			$queryDeleteGeoR="DELETE from CENACOM.GEOR where ID_REPORTE=".$CR_borrar;
			$stidDeleteGeoR = oci_parse($conOracle, $queryDeleteGeoR);
			$r2Delete = oci_execute($stidDeleteGeoR);	
			if (!$r2Delete) {
    			$f = oci_error($stidDeleteGeoR);
				
    			trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
			}else{
				//print "- Estado borrado satisfactoriamente.<br/>";
			}	

		}
/**Fin borrar estados y municipios**/		
/***BORRAR ARCHIVOS***/
