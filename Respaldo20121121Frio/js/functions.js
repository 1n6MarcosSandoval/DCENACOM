...
function alertNumCaracTextArea(campo, numeroMaxCaracteres){
	...
	if(document.getElementById(campo).value.length >= numeroMaxCaracteres)
	{
			...
			document.getElementById(campo+'Leyenda').innerHTML =document.getElementById(campo).title+':<div '+estilo+' >'+'Excedido m&aacute;ximo de caracteres'+'</div>';
			return false; 
	}else{
...
function escribeLinkAlReporte(CR,tipoReporte){
	...
	if(CR>0){
		switch(tipoReporte)
		{
			...
			break;
			case "FINAL":
				document.getElementById('FINAL').innerHTML='<a href="formFinal.php?cr='+CR+'">Generar reporte</a>';
			break;
			case "ALCANCE":
				document.getElementById('ALCANCE').innerHTML='<a href="formAlcance.php?cr='+CR+'">Generar reporte</a>';
			break;
			default:
