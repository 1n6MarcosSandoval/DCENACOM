...
<?php
	...
	echo '</td>';
	echo '<td width="25%">';
	$query1 = oci_parse($conOracle, "SELECT ID_EVENTO FROM CENACOM.EVENTO WHERE NOMBRE_EVENTO='INICIAL' OR NOMBRE_EVENTO='SEGUIMIENTO' AND ESTADO_EVENTO=1 ORDER BY ID_EVENTO DESC");
	oci_execute($query1);
	...
	while ($row = oci_fetch_array($query1, OCI_NUM+OCI_RETURN_NULLS)) {
		foreach ($row as $item)
			$query2 = oci_parse($conOracle, "SELECT MAX(CR) FROM CENACOM.REPORTES WHERE ID_EVENTO=".htmlentities($item)." ORDER BY CR DESC");
			oci_execute($query2);
			while ($row2 = oci_fetch_array($query2, OCI_NUM+OCI_RETURN_NULLS)) {
				foreach ($row2 as $item2)
					print '<option value="'.htmlentities($item2).'">'.htmlentities($item2).'</option>';
