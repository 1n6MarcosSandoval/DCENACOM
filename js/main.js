            ...
            toolbar.startup().then(lang.hitch(this, function () {
                ...
                for (var i = 0; i < this.config.tools.length; i++) {
                    switch (this.config.tools[i].name) {
                        case "legend":
                            toolList.push(this._addLegend(this.config.tools[i], toolbar, "large"));
                            break;
                        case "bookmarks":
                        ...
                        */    break;
                        case "basemap":
                            ...
                            break;
                        case "overview":
                            toolList.push(this._addOverviewMap(this.config.tools[i], toolbar, "large"));
                            break;
                        case "measure":
        ...
        _addLegend: function (tool, toolbar, panelClass) {
			...
			var legendDiv = toolbar.createTool(tool, panelClass);
                    ...
                    domConstruct.create("div", {
                        //id: "panelLogo",
                        innerHTML: "<img id='logo' src='images/Leyenda.jpg' width='100%'></>"
                    },legendDiv));
            ...
            if (layers.length === 0) {
                ...
                if (has("legend")) {
                    ...
                    domConstruct.create("div", {
                        //id: "panelLogo",
                        innerHTML: "<img id='logo' src='images/Leyenda.jpg' width='70%'></>"
                    },legendDiv));
...
require([
    ...
    featureLay = new esri.layers.FeatureLayer("http://servicios1.cenapred.unam.mx:6080/arcgis/rest/services/ANR/Datos_Basicos/MapServer/5", {
    });
    featureLay.id = "FL";
...
function procesar() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var queryTask = new esri.tasks.QueryTask("http://servicios1.cenapred.unam.mx:6080/arcgis/rest/services/ANR/Datos_Basicos/MapServer/5");
            //initialize query
           ...
           // Map2.addLayer(graphicsLayer);
            for (x = 0; x < DatosPintar.Eventos.length; x++) {
                var defaultSymbol;
				var Fenomeno = DatosPintar.Eventos[x]['Fenomeno'];
                switch (DatosPintar.Eventos[x]['nivel']) {
                    case "1":
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'+Fenomeno+'_Alertas_rojo.gif', 50, 50);
                        break;
                    case "2":
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'+Fenomeno+'_Alertas_amarillo.gif', 50, 50);
                        break;
                    case "3":
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'+Fenomeno+'_Alertas_verde.gif', 50, 50);
                        break;
                    default:
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'+Fenomeno+'_Alertas_rojo.gif', 50, 50);
                        break;
                ...
                for (var n in DatosPintar.DatosPol) {
                    for(var m=0, u=DatosPintar.DatosPol[n]['UbicacionPoligonos'].length; m<u; m++){
                        if((DatosPintar.DatosPol[n]['UbicacionPoligonos'][m]['clavemun']==graphic.attributes["CVE_MUN"]) && (DatosPintar.DatosPol[n]['UbicacionPoligonos'][m]['claveestado']==graphic.attributes["CVE_ENT"])  ){
							...
							else if(DatosPintar.DatosPol[i]['DatosGeneralesPol']['nivel']==0){
                                graphic.setSymbol(symbolRojo);
                                //break;							
							}
                            graphic.setInfoTemplate(infoTemplate);
