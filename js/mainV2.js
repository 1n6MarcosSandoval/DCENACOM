    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (x = 0; x < DatosPintar.Eventos.length; x++) {
                var defaultSymbol;
				fenom = ["CLASIFICACIONFENOMENO_ID"];
                switch (DatosPintar.Eventos[x]['nivel']) {
                    case "1":
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'+fenom+'_Alertas_rojo.gif', 30, 30);
                        break;
                    case "2":
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'fenom'_Alertas_amarillo.gif', 30, 30);
                        break;
                    case "3":
                        defaultSymbol = new esri.symbol.PictureMarkerSymbol('img/Simbolos/'fenom'_Alertas_verde.gif', 30, 30);
                        break;
                    default:
