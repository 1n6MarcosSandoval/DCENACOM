...
function autoresLista(){
	...
	j=0;
	for(i=0;i<fseguimiento.seguimientoAutoresC.options.length;i++){
    	if(fseguimiento.seguimientoAutoresC[i].selected==true){
        	arrayAutores[j]=fseguimiento.seguimientoAutoresC[i].value;
        	j++;
    	}    
	}
	cadenaAutores=arrayAutores.join();	
	document.getElementById('seguimientoAutores').value=cadenaAutores;	
}

function valida(){	
	//Construye cadena de links y valida cada uno
	validaLinks=valuesAnidados_nivel2('link','T','seguimientoLinks', 'seguimientoTituloLinks');
	if(!validaLinks){
		return 0;
	}
	...
	//No nulos
	if(document.getElementById('seguimientoAutoresC')){
		if(noNuloAutor('seguimientoAutoresC')){
			return 0;
		}else{
			//Construye la lista de autores en el campo seguimientoAutores
			autoresLista();
			//Recupera el turno de los autores
			document.getElementById('seguimientoAutoresTurno').value=document.getElementById('seguimientoAutoresTurnoC').value;
		}
	}
...
/*	if(noNulo('seguimientoDanosMaterialesEvento')){
		return 0;
	}
	if(noNulo('seguimientoObservacionesEvento')){
		return 0;
	}*/	
	//FIN No nulos
	...
	if(noCero('seguimientoOrganismoReporta')){
		return 0;
	}	
	//FIN No Cero
	...
	if(!validarHora('seguimientoHoraInicialFenomenoval')){
		return 0;
	}
