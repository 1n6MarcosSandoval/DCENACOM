...
function autoresLista(){
	...
	j=0;
	for(i=0;i<finicial.inicialAutoresC.options.length;i++){
    	if(finicial.inicialAutoresC[i].selected==true){
        	arrayAutores[j]=finicial.inicialAutoresC[i].value;
        	j++;
    	}    
	}
	cadenaAutores=arrayAutores.join();	
	document.getElementById('inicialAutores').value=cadenaAutores;	
}

function valida(){	
	//Construye cadena de links y valida cada uno
	validaLinks=valuesAnidados_nivel2('link','T','inicialLinks', 'inicialTituloLinks');
	if(!validaLinks){
		return 0;		
	}
	...
	//No nulos
	if(document.getElementById('inicialAutoresC')){
		if(noNuloAutor('inicialAutoresC')){
			return 0;
		}else{
			//Construye la lista de autores en el campo inicialAutores
			autoresLista();
			//Recupera el turno de los autores
			document.getElementById('inicialAutoresTurno').value=document.getElementById('inicialAutoresTurnoC').value;
		}	
	}
	...
	/*
	if(noNulo('inicialDanosMaterialesEvento')){
		return 0;
	}
	*/
	//FIN No nulos
	...
	if(noCero('inicialOrganismoReporta')){
		return 0;
	}	
	//FIN No Cero
