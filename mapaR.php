...
$linkGEORSS="http://www.incendiosmovil.gob.mx/dcenacom/georssM.php?value=".$llave;
  ...
  require([
    ...
    domStyle
        ...
        function init(){
			  ...
			  var georss = new esri.layers.GeoRSSLayer(georssUrl,{id:"georssReportes"});
              ...
              georss.on("load", function() {
				...
				renderer.addValue("1", symbol_rojo);
				renderer.addValue("2", symbol_naranja);
				renderer.addValue("3", symbol_amarillo);

                // create an info template
                var template = new InfoTemplate("${name}", "${description}<br/> </b><a href=\"${link}\" target=_blank> Más información </a><br/>");
