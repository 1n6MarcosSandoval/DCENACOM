	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function pregSplit($pattern, $modifiers, $subject, $limit=NULL, $flags=NULL) {
		...
		if ((strpos($modifiers, 'u') === FALSE) OR (count(preg_split('//u', "\n\t", -1, PREG_SPLIT_NO_EMPTY)) == 2)) {
			//var_dump($subject);
			//var_dump($pattern.$modifiers);
			$chars = preg_split($pattern.$modifiers, $subject, $limit, $flags);
			//var_dump($chars); 
			//echo 'Separacion';
			$longitud = strlen($subject);
			$chats = array();
			for ($i=0; $i<$longitud ;$i++){
				if($subject[$i] !== ''){
					$conversion =utf8_encode($subject[$i]);
					//echo 'Letra:'.$subject[$i].'Conversion:'.$conversion.' termina';
					if($subject[$i]=='Ã')
					{
						$valorInc =$i+1;
						if($valorInc < $longitud){
							if($subject[$i+1]=='‰'||$subject[$i+1]=='“'||$subject[$i+1]=='š'||$subject[$i+1]=='¡'||$subject[$i+1]=='©'||$subject[$i+1]=='­'||$subject[$i+1]=='³'||$subject[$i+1]=='º'){
								$conversion = $subject[$i].$subject[$i+1];
								$i++;
							}
						}
					}
			   		array_push($chats,$conversion);
				}
			} 
			//var_dump($chats);
			return $chars;
