...
class Archivo{	
	...
	public function upLoadFile(){		
		if($this->checkSize()==false){
			echo '<script>alert("El tamano del archivo sobrepasa el permitido que es de :'.round(($this->tamanoMaximo/1048576),2).'MB");</script>';
			return false;
		}
		if($this->checkType()==false){
			echo '<script>alert("El archivo no corresponde a un formato permitido. Los permitidos son: '.(implode(",",$this->tipoPermitido)).'");</script>';
			return false;
		}
		if(file_exists($this->directorio.$this->archivo)){
			echo '<script>alert("MODIFIQUE EL NOMBRE DEL ARCHIVO");</script>';
			return false;		
		}
