...
$queryS="SELECT MUERTOS, LESIONADOS, DESAPARECIDOS, EVACUADOS,CR, ESTADO, MUNICIPIO, TIPOFENOMENO_ID, EFECTO_ADVERSO, FECHA, ID_USUARIO FROM (SELECT MUERTOS, LESIONADOS, DESAPARECIDOS, EVACUADOS, CR, ESTADO, MUNICIPIO, TIPOFENOMENO_ID, EFECTO_ADVERSO, FECHA, ID_USUARIO, ROWNUM r FROM (SELECT MUERTOS, LESIONADOS, DESAPARECIDOS, EVACUADOS, CR, ESTADO, MUNICIPIO, TIPOFENOMENO_ID, EFECTO_ADVERSO, TO_CHAR(FECHA_AVISO, 'YYYY/MM/DD hh24:mm:ss') AS FECHA, ID_USUARIO FROM CENACOM.REPORTES ORDER BY CR DESC) WHERE ROWNUM <= ".$RegistrosAMostrarQuery." ) WHERE r > ".$RegistrosAEmpezar;
...
echo "<table border='1px' cellpadding='4px' width='760px'>";
	...
	echo '<td bgcolor="#D8D8D8" width="15%"> <center>Muertos</center></td>';
	echo '<td bgcolor="#D8D8D8" width="15%"> <center>Lesionados</center></td>';
	echo '<td bgcolor="#D8D8D8" width="15%"> <center>Desaparecidos</center></td>';
	echo '<td bgcolor="#D8D8D8" width="15%"> <center>Evacuados</center></td>';
	echo '</tr>';
...
//		$firephp->log("Municipio: ".recuperaCampo($conOracle, 'ANRO.LOCALIDADES', 'NOM_MUN', $row["MUNICIPIO"], 'MUN'));
	...
	echo "<td>".$row["MUERTOS"]."</td>";
	echo "<td>".$row["LESIONADOS"]."</td>";
	echo "<td>".$row["DESAPARECIDOS"]."</td>";
	echo "<td>".$row["EVACUADOS"]."</td>";
	echo "</tr>";
}
echo "</table>";
