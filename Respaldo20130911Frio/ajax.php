...
if($_combo=='estados'){
	...
	while ($row = oci_fetch_array($query, OCI_ASSOC)) {
		...
		$_arreglo[] = array('id' => $row['MUN'], 'data' => utf8_encode($row['NOM_MUN']));
	}
	...
	oci_execute($query);
	while ($row = oci_fetch_array($query, OCI_ASSOC)) {
		$_arreglo[] = array('id' => $row['LOC'], 'data' => utf8_encode($row['NOM_LOC']));
	}
	echo json_encode($_arreglo);
	...
	oci_execute($query);
	while ($row = oci_fetch_array($query, OCI_ASSOC)) {
		$_arreglo[] = array('id' => $row['ID_FENOMENO'], 'data' => utf8_encode($row['NOMBRE']));
	}
	echo json_encode($_arreglo);	
	...
	while ($row = oci_fetch_array($query, OCI_ASSOC+OCI_RETURN_NULLS)) {
		$_arreglo[] = array('id' => $row['ID_USUARIO'], 'data' => utf8_encode($row['NOMBRE'])." ".utf8_encode($row['APELLIDO']));
	}
	echo json_encode($_arreglo);	
}
