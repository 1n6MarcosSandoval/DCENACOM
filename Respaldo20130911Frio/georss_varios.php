...
if (!$resultadoEnOracleGEORSS) {
	...
	$ReportesRango=$row0["MAXIMO"]-28;
	//$ReportesRango=90;

	$query1 = oci_parse($conOracle1, "SELECT TO_CHAR(FECHA_AVISO, 'HH24:MI DD-MON-YYYY') AS FECHA, CR, CLAVE_LUGAR, TIPOFENOMENO_ID, PERSONAS_AFECTADAS, MUERTOS, LESIONADOS, DESAPARECIDOS, EVACUADOS, EFECTO_ADVERSO, OBSERVACIONES from cenacom.reportes where ID_REPORTE >".$ReportesRango." ORDER BY CR DESC");
	...
	while ($row1 = oci_fetch_array($query1, OCI_ASSOC+OCI_RETURN_NULLS)) {
		...
		/*******************************************************************************************/
			$lugares = explode(",", $row1["CLAVE_LUGAR"]);
			//$lugares = explode(",","270110015,270110004,270110037,270030048,100340340");
		
			//count($lugares);
			for($i=0;$i<count($lugares);$i++){
				$latitud=recuperaCampo($conOracle1,"coordenadas","LATITUD",$lugares[$i], "CLVE_LUGAR");
				$longitud=recuperaCampo($conOracle1,"coordenadas","LONGITUD",$lugares[$i], "CLVE_LUGAR");
				$estado=recuperaCampo($conOracle1,"coordenadas","ESTADO",$lugares[$i], "CLVE_LUGAR");
				$municipio=recuperaCampo($conOracle1,"coordenadas","MUNICIPIO",$lugares[$i], "CLVE_LUGAR");
				print "\n<item>\n";
				print "<title>";
				$titulo=recuperaCampo($conOracle1, 'CENACOM.TIPO_FENOMENO', 'NOMBRE', $row1['TIPOFENOMENO_ID'], 'ID_FENOMENO');
				print $titulo;
				print "</title>\n";
