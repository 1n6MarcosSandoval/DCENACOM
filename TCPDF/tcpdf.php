...
class TCPDF {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		$chars = TCPDF_FONTS::UTF8StringToArray($s, true, $this->CurrentFont);
	...
	protected function getHtmlDomArray($html) {
		...
		$html = preg_replace('/'.$this->re_space['p'].'+<\/(td|th|li|dt|dd)>/', '</\\1>', $html);
		$html = preg_replace('/[\s]+<(tr|td|th|li|dt|dd)/', '<\\1', $html);
		$html = preg_replace('/'.$this->re_space['p'].'+<(ul|ol|dl|br)/', '<\\1', $html);
		...
		$html = preg_replace('/'.$this->re_space['p'].'+<img/', chr(32).'<img', $html);
		...
		$html = preg_replace('/<li([^\>]*)>'.$this->re_space['p'].'*<img/', '<li\\1><font size="1">&nbsp;</font><img', $html);
		...
		$html = preg_replace('/'.$this->re_space['p'].'+/', chr(32), $html); // replace multiple spaces with a single space
		// trim string
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag']) {
				...
				//if(strpos($dom[$key]['value'],'Eva')!== false)
				///{
				//var_dump($dom[$key]['value']);
				////var_dump (isRTLTextDir());
				//}
				if ((!$this->premode) AND $this->isRTLTextDir()) {
					// reverse spaces order
					$lsp = ''; // left spaces
					$rsp = ''; // right spaces
					if (preg_match('/^('.$this->re_space['p'].'+)/', $dom[$key]['value'], $matches)) {
						$lsp = $matches[1];
					}
					if (preg_match('/('.$this->re_space['p'].'+)$/', $dom[$key]['value'], $matches)) {
						$rsp = $matches[1];
				...
				if ($newline) {
					...
					$dom[$key]['value'] = preg_replace('/^'.$this->re_space['p'].'+$/', chr(32), $dom[$key]['value']);
				}
	...
	*/
	...
	public function stringLeftTrim($str, $replace='') {
		$remplazado= preg_replace('/'.$this->re_space['p'].'+$/', $replace, $str);
		return $remplazado;
	}
	...
	public function stringRightTrim($str, $replace='') {
		$stringRemplazo=preg_replace('/'.$this->re_space['p'].'+$/', $replace, $str);
		return $stringRemplazo;
	}
